...
const Time = () => {
  ...
  const convertTime = async () => {
    try{
    const result = await axios.get(`https://timezone.abstractapi.com/v1/convert_time`, {params: {api_key: accessKey, base_location: firstLocation, base_datetime: firstTime, target_location: secondLocation}})
    if(result.data.target_location){
      setSecondTime(result.data.target_location.datetime + ' (' + result.data.target_location.timezone_abbreviation + ') ')
    }else{
  ...
  return(
    ...
    <div className='time-container'>
      <div className='request-time-container'>
        <h3 className='request-time-text'>Get the crurent time of a location:</h3>
       ...
       {requestTime !== '' ? <div className='request-result-container'>
{requestTime}
        </div> : null}
      </div>
      ...
      <input placeholder='Type the location(e.g: Colts Neck, NJ)' className='from-location-input' onChange={convertLocationHandler}/>
      </div>

      <div className='convert-time-container'>
      <h3 className='from-location-text'>From Time: </h3>
      <input placeholder='2023-07-26 15:39:49' className='from-location-input' onChange={convertTimeHandler}/>
      </div>

      <div className='convert-time-container'>
      <h3 className='from-location-text'>Destination: </h3>
      <input placeholder='Type the location(e.g: Colts Neck, NJ)' className='from-location-input' onChange={convertDestinationHandler}/>
      </div>

      <div className="globe-container">
        <button className='globe-button' onClick={globeClickHandler}><BsGlobeAsiaAustralia size='3em'/></button>
      </div>

    <div className='convert-result-container'>
    <h3 className='from-location-text'>Result: </h3>
    {secondTime !== '' ? <div className='convert-container'>
{secondTime}
        </div> : null}
    </div>
    </div>
